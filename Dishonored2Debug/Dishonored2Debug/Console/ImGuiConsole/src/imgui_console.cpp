...
void ImGuiConsole::Draw()
{
    ...
    if (!ImGui::Begin(m_ConsoleName.data(), nullptr, ImGuiWindowFlags_MenuBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar))
    {
        ImGui::PopStyleVar();
        ImGui::End();
        return;
...
void ImGuiConsole::LogWindow()
{
    ...
    if (ImGui::BeginChild("ScrollRegion##", ImVec2(0, -footerHeightToReserve), false, 0))
    {
        ...
        for (const auto &item : m_ConsoleSystem.Items())
        {
            ...
            // Time stamp.
            if (item.m_Type == csys::kCommand && m_TimeStamps)
            {
...
void ImGuiConsole::InputBar()
{
    ...
    if (ImGui::InputText("Input", &m_Buffer, inputTextFlags, InputCallback, this))
    {
        ...
        if (!m_Buffer.empty())
        {
            // Forward the command line to the game execution function.
            DH2::g_idCmdSystemLocal->ExecuteCommand(m_Buffer.c_str());
    ...
    switch (data->EventFlag)
    {
        case ImGuiInputTextFlags_CallbackCompletion:
        {
            ...
            if (!trim_str.empty())
            {
                ...
                {
                    console->m_ConsoleSystem.Log(csys::kCommand) << "Suggestions: " << csys::endl;

                    for (const auto &suggestion : console->m_CmdSuggestions)
                        console->m_ConsoleSystem.Log(csys::kLog) << suggestion << csys::endl;
